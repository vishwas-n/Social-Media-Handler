...
class Twitter:
    ...
    def get_tweets_for_keyword(self, keyword):
        ...
        for data in tweepy.Cursor(self.api.search_tweets, q=query, count=10, tweet_mode="extended", lang="en", include_entities=True).pages(1):
            print("Fetching {count} tweets".format(count=10))
            for tweet in data:
